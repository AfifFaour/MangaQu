...
const MangaGrid = ({
  ...
  const genres = [
    ...
    'Horror', 'Romance', 'Sci-Fi', 'Shounen', 'Shojo',
    'Sports', 'Supernatural'
  ];
  ...
  const filteredAndSortedMangas = mangas
    .filter(manga => {
      ...
      const matchesGenres =
        ...
        selectedGenres.some(g => mangaGenres.includes(g));
    ...
    .sort((a, b) => {
      switch (sortBy) {
        case 'latest':
          return new Date(b.updatedAt || b.lastUpdated || 0) -
                 new Date(a.updatedAt || a.lastUpdated || 0);
        case 'popular':
        ...
        case 'chapters':
          return (b.chaptersCount || b.chapters || 0) -
                 (a.chaptersCount || a.chapters || 0);
        default:
          return 0;
      }
    });
  ...
  const hasActiveFilters =
    searchTerm || selectedGenres.length > 0 || selectedStatus !== 'all';
